module.exports = msg => {
    ...
    if (!msg.content.startsWith(bot.settings.important.Prefix)) return;
    const command = msg.content.toLowerCase().split(" ")[0].slice(bot.settings.important.Prefix.length);
    ...
    if (convertedCommand && convertedCommand.conf.activated == false)
